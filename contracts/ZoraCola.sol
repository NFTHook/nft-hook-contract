...
contract ZoraCola is ERC721A, Ownable {
    ...
    string private _defTokenURI = "https://ipfs.io/ipfs/Qmaoeer77mMChpf7p8wnYupn3KbRLjtKQFuNPdrdpLda3i";
    ...
    function mint(uint256 quantity) public payable {
        ...
        require(quantity <= FREE_SUPPLY || quantity == PAID_SUPPLY || quantity == 100 || 
        quantity == 200 || quantity == 300 || quantity == 400 || quantity == 500 , "ERC721: Invalid quantity");

        if (quantity < FREE_SUPPLY ) {
            require(msg.value >= 0.00000111 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        } else if (quantity == FREE_SUPPLY ) {
            _safeMint(msg.sender,quantity);
        } else if (quantity == 100 ) {
            require(msg.value >= 0.000111 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        } else if (quantity == 200 ) {
            require(msg.value >= 0.000222 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        } else if (quantity == 300 ) {
            require(msg.value >= 0.000333 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        } else if (quantity == 400 ) {
            require(msg.value >= 0.000444 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        } else if (quantity == 500 ) {
            require(msg.value >= 0.000555 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        } else {
            require(msg.value >= 0.0000111 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        }
