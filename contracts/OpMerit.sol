...
contract OpMerit is ERC721A, Ownable {

    uint256 public constant FREE_SUPPLY = 3;
    uint256 public constant PAID_SUPPLY = 10;

    uint256 private _flag;
    string private _defTokenURI = "https://ipfs.io/ipfs/QmWTvXegaXPe9p7cN4ZZ1sj7NnXNRFGQNwsx3DGdvjmBQD";
    ...
    constructor() Ownable(msg.sender) ERC721A("OP Merit-Manjushri's Protection", "OMM") {
    }
    ...
    function mint(uint256 quantity) public payable {
        ...
        if (quantity < FREE_SUPPLY ) {
            require(msg.value >= 0.00013568 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        } else if (quantity == FREE_SUPPLY ) {
            _safeMint(msg.sender,quantity);
        } else if (quantity == 100 ) {
            require(msg.value >= 0.00086531 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        } else if (quantity == 200 ) {
            require(msg.value >= 0.00173062 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        } else if (quantity == 500 ) {
            require(msg.value >= 0.00432655 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        } else {
            require(msg.value >= 0.000086531 ether, "ERC721: Insufficient payment");
            _safeMint(msg.sender,quantity);
        }
